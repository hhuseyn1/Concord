...
import { MentionAutocomplete } from '../messages/MentionAutocomplete'
import { mapAttachmentUploadError } from '../messages/messagesErrors'
import { useMentionAutocomplete } from '../messages/useMentionAutocomplete'
import { clearDraft, readDraft, useDraftPersistence } from '../messages/useDraftPersistence'
import { mapSendDirectMessageError } from './directMessagesErrors'
import { useSendDirectMessageMutation } from './directMessagesQueries'
import { useOtherUserProfile } from './useOtherUserDisplayName'
...
export function DirectMessageComposer({
  ...
  const draftKey = conversationId ? `dm:${conversationId}` : undefined
  ...
  const otherUserProfile = useOtherUserProfile(conversationId)
  const mentionCandidates = useMemo(() => (otherUserProfile ? [otherUserProfile] : []), [otherUserProfile])
  ...
  const sendMutation = useSendDirectMessageMutation(conversationId)
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast({ variant: 'danger', title: 'Could not send message', description: mapSendDirectMessageError(error) })
    }
  }
    ...
    >
      ...
      <div className="relative flex items-end gap-2 rounded-md border border-border-default bg-surface-sidebar px-2 py-1.5 focus-within:border-brand">
        ...
        <Textarea
          ...
          onPaste={handlePaste}
          placeholder={otherUserDisplayName ? `Message @${otherUserDisplayName}…` : 'Message…'}
          rows={1}
          disabled={sendMutation.isPending}
          invalid={overLimit}
