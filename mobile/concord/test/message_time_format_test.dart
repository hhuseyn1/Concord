...
void main() {
  ...
  test('formatShortTime renders a 12-hour "H:MM AM/PM" string', () {
    ...
    expect(formatShortTime(l10n, utc), matches(RegExp(r'^\d{1,2}:\d{2} (AM|PM)$')));
  });

  test('formatGroupTimestamp labels a just-now message as "Today"', () {
    final now = DateTime.now().toUtc();
    expect(formatGroupTimestamp(l10n, now), startsWith('Today at '));
  });
  ...
  test('formatGroupTimestamp labels a message from exactly a day ago as "Yesterday"', () {
    ...
    final formatted = formatGroupTimestamp(l10n, yesterday);
  ...
  test('formatAbsoluteTimestamp renders a full date + time', () {
    ...
    expect(formatAbsoluteTimestamp(utc), matches(RegExp(r'^\d{2}\.\d{2}\.\d{4}, \d{2}:\d{2}$')));
  });
}
