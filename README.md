...
cd backend/Concord
docker build -t shared:Local -f Dockerfile .
docker compose --profile db --profile backend --profile frontend up -d --build
```

Open the frontend at `http://localhost:${FRONTEND_HTTP_PORT}` (`5173` by default in `.env.Local`).